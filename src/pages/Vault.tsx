...
export const Vault = ({
  ...
  state: ReturnType<typeof useSystemState>;
}) => {
  void state;
  const { can } = useAuth();
  ...
  const allCredentials: VaultItem[] = [
    ...vault.accounts.map((a) => ({
      ...
      creds: (a.credentials as Record<string, unknown>) ?? {},
      type: String(a.type ?? ""),
      has2FA: !!(a.credentials as Record<string, unknown> | undefined)
        ?.twoFactor,
      section: "accounts" as VaultSection,
    })),
    ...vault.hardware
      .filter((h) => !!h.credentials)
      .map((h) => ({
        id: `hw-${String(h.id)}`,
        name: String(h.name ?? ""),
        source: "Hardware",
        email: "Device Access",
        creds: h.credentials as Record<string, unknown>,
        type: "Local",
        has2FA: false,
        section: "hardware" as VaultSection,
      })),
    ...vault.tools
      ...
      .map((t) => ({
        ...
        email: String(
          (t.credentials as Record<string, unknown>)?.email ?? "N/A",
        ),
        creds: t.credentials as Record<string, unknown>,
        type: "Standalone",
        has2FA: false,
        section: "tools" as VaultSection,
      })),
    ...vault.subscriptions
      .filter(
        (s) =>
          !!(s.credentials as Record<string, unknown> | undefined)?.password,
      )
      .map((s) => ({
        id: `sub-${String(s.id)}`,
        name: String(s.name ?? ""),
        source: "Subscription",
        email: String(
          (s.credentials as Record<string, unknown>)?.email ?? s.vendor,
        ),
        creds: s.credentials as Record<string, unknown>,
        type: String(s.type ?? ""),
        has2FA: false,
        section: "subscriptions" as VaultSection,
      })),
    ...vault.projects.flatMap((p) =>
      ((p.standaloneCredentials as Array<Record<string, unknown>>) ?? []).map(
        (sc) => ({
          id: `sc-${String(sc.id)}`,
          name: `${String(p.name ?? "")} → ${String(sc.label ?? "")}`,
          source: "Project Credential",
          email: String(sc.username ?? p.clientName ?? ""),
          creds: {
          ...
          type: "Project",
          has2FA: false,
          section: "projects" as VaultSection,
          url: sc.url,
          notes: sc.notes,
        }),
      ),
  ...
  const sectionsWithCount: {
    ...
    },
    {
      ...
      label: "Accounts",
      icon: <ShieldCheck className="w-3.5 h-3.5" />,
      count: vault.accounts.length,
    },
    {
      id: "hardware",
      label: "Hardware",
      icon: <Key className="w-3.5 h-3.5" />,
      count: vault.hardware.filter((h) => !!h.credentials).length,
    },
    {
      id: "tools",
      label: "Tools",
      icon: <Key className="w-3.5 h-3.5" />,
      count: vault.tools.filter((t) => !!t.credentials).length,
    },
    {
      id: "subscriptions",
      label: "Subscriptions",
      icon: <CreditCard className="w-3.5 h-3.5" />,
      count: vault.subscriptions.filter(
        (s) =>
          !!(s.credentials as Record<string, unknown> | undefined)?.password,
      ).length,
    },
    {
      ...
      count: vault.projects.reduce(
        (n, p) =>
          n +
          ((p.standaloneCredentials as Array<Record<string, unknown>>) ?? [])
            .length,
        0,
      ),
    },
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">
            Secure system-wide access to all stored credentials and 2FA codes.
          </p>
        </div>
      </div>
        ...
        <div className="flex items-center gap-3 p-4 rounded-2xl bg-amber-500/5 border border-amber-500/20">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Your role does not allow revealing passwords. Contact an
          ...
          <Card className="p-4 space-y-4">
            <div>
              ...
              <div className="h-2 w-full bg-accent rounded-full overflow-hidden">
                <div
                  ...
                  style={{
                    width: `${Math.round((accountsWith2FA / Math.max(vault.accounts.length, 1)) * 100)}%`,
                  }}
                />
              </div>
              <p className="text-[10px] mt-1 text-muted-foreground font-medium">
                {accountsWith2FA}/{vault.accounts.length} accounts have 2FA
              </p>
            </div>
            ...
            <div className="space-y-2 pt-2 border-t text-sm">
              ...
              <div className="flex justify-between">
                ...
                <Badge variant={canReveal ? "success" : "warning"}>
                  {canReveal ? "Full" : "Read-Only"}
                </Badge>
              </div>
            </div>
            ...
            <div className="pt-2 border-t">
              ...
              <div className="flex lg:flex-col gap-1 overflow-x-auto lg:overflow-visible pb-2 lg:pb-0 custom-scrollbar scrollbar-hide lg:scrollbar-default">
                {sectionsWithCount.map((s) => (
                  <button
                    key={s.id}
                    onClick={() => setSection(s.id)}
                    className={`flex items-center justify-between gap-3 px-3 py-2 rounded-xl text-xs sm:text-sm font-medium transition-all shrink-0 lg:shrink lg:w-full ${section === s.id ? "bg-primary/20 text-primary border border-primary/20" : "hover:bg-accent text-muted-foreground border border-transparent"}`}
                  >
        ...
        <div className="xl:col-span-3 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            {filtered.map((item) => (
              <Card
                key={item.id}
                className="p-4 hover:shadow-premium transition-all space-y-4"
              >
                <div className="flex flex-col sm:flex-row justify-between items-start gap-4">
                  <div className="flex items-center gap-3">
                    <div
                      className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${sectionColor[item.section]}`}
                    >
                      {item.section === "accounts" && (
                        <ShieldCheck className="w-5 h-5" />
                      )}
                      {item.section === "hardware" && (
                ...
                <div className="space-y-3">
                          ...
                          <Button
                            ...
                            onClick={async () => {
                              try {
                                ...
                                const plain = await revealMutation.mutateAsync(
                                  item.creds.password as EncryptedValue,
                                );
                                ...
                                pushToast(
                                  "success",
                                  `Password revealed for ${item.name}.`,
                                );
                              } catch (error) {
                  ...
                  {(Array.isArray(item.creds.customFields)
                    ? item.creds.customFields
                    : []
                  ).map((cf: any, i: number) =>
                    canReveal ? (
                      <CredentialField
